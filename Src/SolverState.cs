...
public abstract class SolverState
{
    ...
    /// <summary>
    ///     Determines if a specific <paramref name="value"/> in a specific <paramref name="cell"/> has already been marked
    ///     impossible or is out of range.</summary>
    public abstract bool IsImpossible(int cell, int value);

    /// <summary>
    ///     Determines if all values that are still possible in the specified <paramref name="cell"/> have the same value when
    ///     projected through the specified <paramref name="predicate"/>. For example, this can be used to determine if all
    ///     the values still possible are the same parity, or all primes/non-primes, etc.</summary>
    /// <param name="cell">
    ...
    /// <summary>
    ///     If not <c>null</c>, <see cref="Constraint.Process(SolverState)"/> was called immediately after placing a value in
    ///     the cell given by this index. (This value may be tentative; if the algorithm finds it to be impossible, it will
    ///     backtrack.) The implementation may examine deductions from just that one value. If <c>null</c>, the method may
    ///     have been called (a) at the very start of the algorithm before placing any values; (b) after this constraint was
    ///     returned from another constraint’s <see cref="Constraint.Process(SolverState)"/> call to replace it; or (c) if the
    ///     constraint has <see cref="Constraint.CanReevaluate"/> and one of its affected cells has changed. In these cases,
    ///     implementations should examine the whole set of affected cells for possible deductions.</summary>
