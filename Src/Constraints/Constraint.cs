...
public abstract class Constraint(IEnumerable<int> affectedCells)
{
    ...
    ///         replaced with (can be empty), or <see cref="ConstraintViolation"/> to indicate that this constraint is already
    ///         violated.</para>
    ///     <para>
    ///         For example, in <see cref="EqualSumsConstraint"/>, since the sum is not initially known, the constraint waits
    ///         until one of its regions is filled and then uses this return value to replace itself with several <see
    ///         cref="SumConstraint"/>s to ensure the other regions have the same sum.</para>
    ///     <para>
    ///         The algorithm will automatically call this method again on all the new constraints for all cells already
    ///         placed in the grid. The constraints returned MUST NOT themselves return yet more constraints at that point.</para></returns>
    ...
    ///     constraint merely rules out a possibility. Derived types can override this and return <c>true</c> to indicate to
    ///     the solver that the constraint should be reevaluated (meaning: have <see cref="Process"/> called on it again) when
    ///     another constraint rules out a value in one of the affected cells of this constraint.</summary>
    ...
    /// <param name="str">
    ///     A string such as <c>"A1-4,B-E1,E2"</c>. The letters are assumed to be columns (starting from A) while the digits
    ///     are rows (starting from 1). Ranges can be combined to create rectangles; thus, <c>A-C1-3</c> generates a 3×3
    ///     square.</param>
